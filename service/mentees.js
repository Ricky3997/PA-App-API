...
const getAll = async (admin) => {
  ...
  return await Mentee.find(criteria).populate({
    path: 'relationship',
...
const getById = async (id) => {
  return await Mentee.findById(id).populate({
    path: 'relationship',
    populate: { path: 'mentor' }
  }).populate({ path: 'mentorBlackList', populate: { path: 'mentor' } }).exec().then(p => {
    return p;
...
const edit = async (id, data, file) => {
  ...
  await Mentee.findByIdAndUpdate(id, data, { new: true }).exec().then(p => {
    return p;
  });
  return Mentee.findById(id).populate({
    path: 'relationship',
    populate: { path: 'mentor' }
  }).populate({ path: 'mentorBlackList', populate: { path: 'mentor' } }).exec().then(p => {
    return p;
...
const registerNew = async (id, data) => {
  ...
  return await User.findByIdAndUpdate(id, {
    onboarded: true,
    menteeProfile: id
  }, { new: true }).populate('menteeProfile').exec().then(p => {
    return p;
  });
...
const changeStatus = async (id, data) => {
  try {
    ...
    return User.findById(id).populate('menteeProfile').exec().then(p => {
      return p;
    });
  } catch (e) {
    return null;
