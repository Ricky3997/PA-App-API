    ...
    <Row>
      <Col md={2}>
        <Nav variant="pills" className="flex-column"
          ...
          <Nav.Item>
            <Nav.Link eventKey="approvals"><span>{"Mentors Pending Approval "}<BadgePendingNumber
              pending={props.admin.mentors.filter(m => m.status === "requested")}/></span></Nav.Link>
          </Nav.Item>
          <Nav.Item>
      ...
      <Col md={10}>
        <Tab.Content>
          <Tab.Pane active>
            <Switch>
              <Route path={"/admin/mentors/approvals/:id?"}
              ...
              <Route path={"/admin/mentors/database/:id?"}
                     render={({ match, history }) => <Database history={history} id={match.params.id} match={match}
                                                               mode="mentors" mentors={mentors}/>}/>
            </Switch>
          </Tab.Pane>
        </Tab.Content>
      </Col>
